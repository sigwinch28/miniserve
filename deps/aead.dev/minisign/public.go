...
func (p PublicKey) Verify(message []byte, signature []byte, opts crypto.SignerOpts) (result bool, err error) {
	switch h := opts.HashFunc(); h {
	case crypto.Hash(0):
		const isHashed = false
		return verify(p, message, signature, isHashed), nil

	case crypto.BLAKE2b_512:
		const isHashed = true
		if n := len(message); n != blake2b.Size {
			return false, errors.New("minisign: invalid message length " + strconv.Itoa(n))
		}
		return verify(p, message, signature, isHashed), nil
	default:
		return false, errors.New("minisign: cannot sign messages hashed with " + strconv.Itoa(int(h)))
	}
}
