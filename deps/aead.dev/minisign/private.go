...
func (p PrivateKey) Sign(_ io.Reader, message []byte, opts crypto.SignerOpts) (signature []byte, err error) {
	...
	return p.SignWithComments(message, trustedComment, untrustedComment, opts)
}

func (p PrivateKey) SignWithComments(message []byte, trustedComment, untrustedComment string, opts crypto.SignerOpts) (signature []byte, err error) {
